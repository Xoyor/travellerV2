...
import java.util.ArrayList; // library ที่ใช้ในการจัดการ list แบบ dynamic สามารถเพิ่มลดขนาดได้ตามต้องการ ถ้า Array แบบปกติจะมีขนาดตายตัว
...
class ExpenseCalculator {
    ...
    double getAverage() {
        // return ค่าใช้จ่ายฉลี่ย ? คือการทำ if else แบบย่อ ถ้า true แล้วจะ เอาค่า
        // getTotal() ที่เป็นเงินรวมที่ใช้และจำนวนคนมาหารกัน
        // getTotal()(เงินรวมทั้งหมด) / people.size()(จำนวนคน)
        // ถ้าเป็น false จะ return 0
    ...
    String getBalanceReport() {
        ...
        for (Person p : people) { // การวน Loop ของ object แต่ละคนทีอยู่ใน Array people
                                  // เพื่อเช็คว่าเงินที่ช่ายไปเท่าไหร่ กับ ค่าเฉลี่ยที่ควรจะจ่าย
            double balance = p.amount - avg; // ค่า balance = จำนวนเงินที่จ่ายไป - ค่าเฉลี่ย เพื่อหาว่าต้องจ่ายเพิ่ม
                                             // หรือได้รับเพิ่มเท่าไร
            if (balance > 0.01)
                sb.append("💚 " + p.name + " should receive " + String.format("%.2f", balance) + " Baht\n");
            else if (balance < -0.01) // ในกรณีที่เป็นลบ -0.01 (เพราะเซ็ตค่าของตัวแปรเป็น double
                                      // เลยต้องกันจากจุดทศนิยม)
                sb.append("💸 " + p.name + " should pay " + String.format("%.2f", -balance) + " Baht\n");
            else // ในกรณีที่ค่าเป็น 0
...
public class TravelExpenseSplitter extends JFrame implements ActionListener {
    ...
    public TravelExpenseSplitter() {
        ...
        // Set up the UI with proper look and feel
        try {
    ...
    private Font getEmojiSupportedFont(int style, int size) {
        // Try to get a font that supports emojis, prioritizing Windows fonts
        String[] fontNames = {
        ...
        for (String fontName : fontNames) {
            ...
            if (font.canDisplay(0x1F600)) { // Test with grinning face emoji
                return font;
            }
        }

        return new Font("SansSerif", style, size); // Final fallback
    }
    ...
    private JPanel createTitlePanel() {
        ...
        subtitleLabel.setFont(getEmojiSupportedFont(Font.ITALIC, 14));
    ...
    private JPanel createLeftPanel() {
        ...
        inputTitle.setFont(getEmojiSupportedFont(Font.BOLD, 16));
        ...
        amountLabel.setFont(getEmojiSupportedFont(Font.BOLD, 14));
        ...
        listTitle.setFont(getEmojiSupportedFont(Font.BOLD, 16));
    ...
    private JPanel createRightPanel() {
        ...
        resultTitle.setFont(getEmojiSupportedFont(Font.BOLD, 16));
        resultTitle.setForeground(new Color(70, 130, 180));

        resultArea = new JTextArea();
        resultArea.setFont(getEmojiSupportedFont(Font.PLAIN, 13));
    ...
    private JButton createStyledButton(String text, Color bgColor) {
        ...
        button.setFont(getEmojiSupportedFont(Font.BOLD, 13));
    ...
    private void addPerson() {
        try {
            ...
            if (amt < 0) {
                JOptionPane.showMessageDialog(this, "Amount cannot be negative!", "Input Error",
                        JOptionPane.WARNING_MESSAGE);
                return;
            }
    ...
    private void removePerson() {
        ...
        if (selectedIndex != -1) {
            ...
            JOptionPane.showMessageDialog(this, "Please select a person to remove!", "Selection Error",
                    JOptionPane.WARNING_MESSAGE);
        }
    }
    ...
    private void calculateSplit() {
        if (people.size() == 0) {
            JOptionPane.showMessageDialog(this, "Please add at least one person first!", "No Data",
                    JOptionPane.WARNING_MESSAGE);
            return;
        }
    ...
    public static void main(String[] args) {
        // Set system properties for better Unicode/emoji support
        System.setProperty("file.encoding", "UTF-8");
